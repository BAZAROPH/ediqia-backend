...
class AuthController extends Controller
{
    ...
    public function index2(Request $request)
    {
       ...
       $connected =   DB::select('select * from sessions where user_id = ? and payload = ? and tokens is not null limit 1', [$user_connecte->id,$macadress]);;
        ...
        $appareril_info = DB::table('sessions')
                            ->where('user_id',$id)
                            //->where('causer_type','=',  'App\Models\User')
                            ->orderBy('id','DESC')
                            ->take(10)
    ...
    public function updateUsers(Request $request)
    {
        ...
        User::find($request->user_id)->update([
            ...
            'date_naissance' => $data['date_embauche'],
    ...
    public function login(Request $request)
    {
       ...
       // $user = User::where('email', $data['email'])->where('password', Hash::check(12345678  ,$data['email'] ))->first(); //Auth::user()
    ...
    public function logout(Request $request)
    {
        ...
        $macAddr = substr(shell_exec('getmac'), 159,20);
        $session = DB::select('select * from sessions where payload = ? and user_id = ? and tokens is not null', [$macAddr,Auth::user()->id]);

        for($i=0;$i<count($session);$i++)
        {
            $session_deleted[$i] =  DB::delete('delete from sessions where payload = ? and user_id = ? and tokens is not null', [$macAddr,Auth::user()->id]);
        }
